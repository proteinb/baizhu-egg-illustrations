...
一句话：**默认画 A。除非文章真的需要 B/C/D/E 来说明意思，否则不要为了好看硬换形态。每组 4-8 张长文配图中，强相关时最多允许 1 张使用 B/C/D/E 主变体，并且必须在 Shot List 写明选择理由。**
...
- 生成 16:9 横版白煮蛋风格插图
- 为口播稿、短视频或剪辑工作流输出视频化分层配图方案
- 将观点、流程、结构、状态或隐喻转成可读但不死板的手绘解释图
- 保持固定视觉 IP：围裙白煮蛋、小工作鞋、黑色手绘线稿、少量红橙蓝中文批注
- 当上游提供 `unit_id` 时，透传语义单元编号，并输出可追溯的 `module_id`、图层建议和 `manifest.json`
...
- 2026-07-02：增加视频化分层配图能力。支持透传上游 `unit_id`、输出语义模块、透明 PNG 分层建议和 `manifest.json`；没有上游 `unit_id` 时必须标记 `unit_id_source: missing`，不能伪造口播稿对齐编号。
- 2026-06-24：强化白煮蛋 IP 锁定。Shot List 和生图提示词必须明确写出正面深色围裙、小结/系带/细竖纹或小口袋、小工作鞋、黑点眼、细手细脚和克制空表情；围裙太小、像腰线或被遮挡都算失败。
- 2026-06-24：补充变体出现频率规则。每组 4-8 张长文配图默认以 A 为主；强相关时最多 1 张使用 B/C/D/E 主变体，并且必须在 Shot List 写明选择理由。
